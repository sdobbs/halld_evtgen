...
void EvtBTo3hCP::EvtSet_Constants(double balpha, double bbeta)
{
  ...
  EvtComplex Mat_Tp0(calpha, -salpha); Mat_Tp0 *= 1.09;
  EvtComplex Mat_Tm0(calpha, -salpha); Mat_Tm0 *= 1.09;
  EvtComplex Mat_T0p(calpha, -salpha); Mat_T0p *= 0.66;
  EvtComplex Mat_T0m(calpha, -salpha); Mat_T0m *= 0.66;
  EvtComplex Mat_Tpm(calpha, -salpha); Mat_Tpm *= 1.00;
  EvtComplex Mat_Tmp(calpha, -salpha); Mat_Tmp *= 0.47;

  EvtComplex Mat_Ppm(cos(-0.5), sin(-0.5)); Mat_Ppm *= -0.2;
  EvtComplex Mat_Pmp(cos(2.), sin(2.)); Mat_Pmp *= 0.15;

  EvtComplex Mat_P1 = 0.5 * (Mat_Ppm - Mat_Pmp);
  EvtComplex Mat_P0 = 0.5 * (Mat_Ppm + Mat_Pmp);
  EvtComplex Nat_Tp0(calpha, salpha); Nat_Tp0 *= 1.09;
  EvtComplex Nat_Tm0(calpha, salpha); Nat_Tm0 *= 1.09;
  EvtComplex Nat_T0p(calpha, salpha); Nat_T0p *= 0.66;
  EvtComplex Nat_T0m(calpha, salpha); Nat_T0m *= 0.66;
  EvtComplex Nat_Tpm(calpha, salpha); Nat_Tpm *= 1.00;
  EvtComplex Nat_Tmp(calpha, salpha); Nat_Tmp *= 0.47;
...
void EvtBTo3hCP::Evt3pi(double alpha, int iset,
                        EvtVector4R &p_pi_plus, EvtVector4R &p_pi_minus,
...
// Ghm : beta is not needed for this generation - put a default value
    ...
    for (int i = 0; i < endLoop; ++i)
    {
      ...
      factor = (1.0 + sqrt(square(R1) + square(R2))) * (AB0 + AB0bar) / 2.0;
...
void EvtBTo3hCP::Evt3piMPP(double alpha, int iset,
                                 ...
                                 double &imag_B0bar, int iset) 
{
  ...
  double W12 = 1. / ((square(Mass_rho - m12) + square(Gam_rho / 2.)) * m12);
  double W13 = 1. / ((square(Mass_rho - m13) + square(Gam_rho / 2.)) * m13);
  double W23 = 1. / ((square(Mass_rho - m23) + square(Gam_rho / 2.)) * m23);
                                 ...
                                 double &imag_B0bar, int iset) 
{
  ...
  EvtComplex Mat_rhop =
      BreitWigner(p1, p2, p3, ierr) + BreitWigner(p1, p3, p2, ierr);
                                 ...
                                 double &imag_B0bar, int iset) 
{
  ...
  double W12 = 1. / ((square(Mass_Kstar0 - m12) + square(Gam_Kstar0 / 2.)) * m12);
  double W13 = 1. / ((square(Mass_Kstarp - m13) + square(Gam_Kstarp / 2.)) * m13);
  double W23 = 1. / ((square(Mass_rho - m23) + square(Gam_rho / 2.)) * m23);
                                   ...
                                   double Width) 
{
  ...
  double costet =
      (2. * p1.get(0) * p3.get(0) - m13sq + p1.mass2() + p3.mass2()) /
      (2. * p1.d3mag() * p3.d3mag());
  ...
  if (pipiMode)
  {
    ...
    {
      double factor = 2 * (square(Mass_rho - m12) + square(0.5 * Gam_rho));
  ...
  else
  {
    if (relatBW)
    {
      ...
      result = coscms * EvtRBW(square(m12), square(Mass), Width, Am2Min);
    }
    else
    {
...
//...rho(770)
  ...
  if (kuhn_santa)
  {
    result = ( EvtcBW_KS(s,square(AmRho),GamRho)           +      //!...BW-rho( 770)
               EvtcBW_KS(s,square(AmRhoP),GamRhoP)*(beta)  +      //!...BW-rho(1450)
               EvtcBW_KS(s,square(AmRhoPP),GamRhoPP)*(gamma) ) /  //!...BW-rho(1700)
               (1.+beta+gamma);
  }
  else
  {
...
double EvtBTo3hCP::h(double s)
{
  ...
  double sqrts = sqrt(s);
  double k_s = k(s);
  return 2. / pi * (k_s / sqrts) * log((sqrts + 2. * k_s) / (2. * AmPi));
}

double EvtBTo3hCP::dh_ds(double s)
{
  const double pi=3.141593;
  return h(s) * (1. / (8. * square(k(s))) - 1. / (2 * s)) + 1. / (2. * pi * s);
}
