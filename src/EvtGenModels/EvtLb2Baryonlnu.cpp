...
void EvtLb2Baryonlnu::initProbMax() {
  ...
  {
    EvtGenReport(EVTGEN_ERROR,"EvtGen") << "Decay does not have acceptable final state baryon for this model setting ProbMax = 0 "
			   <<endl;
    setProbMax(0.0);
  }
...
void EvtLb2Baryonlnu::init(){
  ...
  if (getNArg()!=4) {
    ...
    EvtGenReport(EVTGEN_ERROR,"EvtGen") << "EvtLb2Baryonlnu generator expected "
